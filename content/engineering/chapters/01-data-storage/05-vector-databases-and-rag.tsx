...
<p>A <strong>vector embedding</strong> is a list of real numbers that represents a piece of content — a word, a sentence, an image, an audio clip — in a high-dimensional space. The useful property is this: <strong>semantic similarity becomes geometric proximity.</strong> Things that mean similar things end up near each other in that space.</p>
<p>The classic demonstration comes from word2vec, an embedding model trained on text in 2013. If you take the vector for "king," subtract the vector for "man," and add the vector for "woman," you get a vector that is closest to "queen":</p>`,
    },
    {
      type: "callout",
      variant: "info",
...
<p>This works because embeddings are trained to place words used in similar contexts near each other. "King" and "queen" appear in similar sentence structures; so do "man" and "woman." The difference vector encodes the concept of gender.</p>`,
    },
    {
      type: "text",
      html: `<p>Modern embeddings come out of transformer models, with typical embedding sizes ranging from 512-3072. Whatever the dimensionality, the storage problem is the same: you need a database that can hold millions of these vectors and answer the question "find me the $k$ most similar to this query vector" in milliseconds.</p>
...
<p>A value of 1 means the vectors point in the same direction (maximally similar). A value of 0 means they're perpendicular (orthogonal/unrelated). A value of −1 means they point in opposite directions.</p><br>
<p>Algorithms like HNSW (Hierarchical Navigable Small World graphs) and IVFFlat allow approximate nearest-neighbor search at scale — finding the $k$ closest vectors quickly without an exhaustive scan.</p>`,
    ...
    {
      type: "text",
      html: `<p>The most prevalent application of vector storage right now is for <strong>Retrieval-Augmented Generation (RAG)</strong>. If you've used an "AI chatbot for our docs" product — a customer support bot, a document Q&A system, an internal knowledge assistant — you've almost certainly used RAG. We will provide a brief introduction here. We will dive into the details in <a href="https://deeplearningworkbook.com" class="underline">Deep Learning</a> next semester.</p>`,
    },
    {
      type: "callout",
      variant: "example",
      ...
      html: `<ol>
<li>1. Take your private knowledge base — your company's docs, your product catalog, your internal wiki — and split it into chunks.</li>
<li>2. Embed each chunk into a vector using an embedding model. Store the vectors in a vector database alongside the original text.</li>
<li>3. When a user asks a question, embed <em>the question</em> with the same embedding model.</li>
<li>4. Query the vector database: find the $k$ chunks most similar to the question vector.</li>
<li>5. Stuff those chunks into the LLM prompt as context: "Given the following documents, answer the question..."</li>
<li>6. The LLM answers — grounded in <em>your</em> data, not its training data.</li>
</ol><br>
<p>LLMs are trained on public data with a knowledge cutoff. They don't know what's in your internal docs, your proprietary research, or anything that happened after their training ended. RAG solves this without fine-tuning, at a fraction of the cost.</p>`,
    },
    {
      type: "image",
      src: "/engineering/rag.png",
      ...
      caption: "Diagram of a RAG system: documents are embedded and stored in a vector database; at query time, the question is embedded, nearest neighbors are retrieved, and results are passed to an LLM",
    },
    {
      type: "callout",
      variant: "warning",
