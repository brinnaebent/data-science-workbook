...
const relationalDatabasesAndSQL: Section = {
  ...
  blocks: [
    {
      type: "text",
      html: `<p>Edgar F. Codd published the relational model in 1970. The databases it inspired — Postgres, MySQL, SQL Server, SQLite — are still the workhorses of structured data half a century later. That's because the relational model is really good at what it does! Data lives in <strong>tables</strong> of rows and columns. Tables relate to each other through <strong>keys</strong>. Queries are expressed in SQL, a language that is approachable enough for analysts and powerful enough for production systems.</p>
<p>Note: SQL will appear in technical interviews, so this is one to review!</p>`,
    },
    {
      type: "callout",
      variant: "info",
...
DELETE FROM employees WHERE department = 'HR';</code></pre>`,
    ...
    {
      type: "text",
      html: `<p>The operation that trips up most people is <strong>JOIN</strong> — combining rows from two or more tables based on a related column. The mental model is set theory, but you don't need to think in sets to use joins fluently. Think instead about what rows you want in your result.</p>`,
    },
    {
      type: "interactive",
      component: "JoinTypesGrid",
      caption: "",
      props: {},
    },
    {
