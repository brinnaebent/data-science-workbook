...
<p>Three platforms define this space. You typically don't pick the warehouse — your employer already has. But understanding the trade-offs makes you useful in architecture conversations, and it can come up in interviews.</p>`,
    },
    {
      type: "callout",
      variant: "info",
...
<br><p>Redshift works well for read-heavy analytical workloads at significant scale. Its Redshift Spectrum feature lets you query data directly in S3 without loading it into the warehouse — a useful bridge between lake and warehouse patterns.</p>`,
    },
    {
      type: "callout",
      variant: "info",
      title: "Google BigQuery",
      html: `<p>BigQuery is serverless — you don't provision clusters or think about nodes. You write SQL, submit queries, pay per byte scanned. This makes it the fastest path from "zero" to "querying petabytes of data" of any warehouse in the market.</p><br>
<p>The billing model is also different from Redshift: you pay per query rather than per cluster-hour, which is cheaper for intermittent workloads but can become expensive if you run large scans frequently. BigQuery has become the default recommendation for learning and prototyping precisely because the operational overhead is near-zero.</p>`,
    ...
    },
    {
      ...
      html: `<p>A healthcare analytics team is moving from an on-premises data warehouse to the cloud. They need strong compliance controls (HIPAA), SQL-based transformations, and an easy bridge to their existing AWS infrastructure. Their team is deep in AWS and doesn't want to manage multiple cloud accounts.</p><br>
<p><strong>Best fit: Redshift.</strong> Native AWS integration, HIPAA-eligible (with proper configuration), columnar storage for the analytical workloads they're running. Snowflake would also work but introduces multi-cloud complexity they don't need. BigQuery would require migrating their AWS infrastructure to GCP or managing cross-cloud access.</p><br>`,
    },
    {
      type: "reflection",
      id: "eng-ch2-s5-r1",
