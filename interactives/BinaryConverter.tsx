    ...
    <div className="rounded-xl border border-slate-700 bg-slate-900 overflow-hidden">
      ...
      <div className="p-5 flex flex-col gap-6">
        ...
        <p className="text-sm text-slate-400 leading-relaxed">
          ...
          <span className="text-emerald-400 font-semibold">ON</span> or{" "}
          <span className="text-slate-500 font-semibold">OFF</span>. Nothing else.
                ...
                <div
                  className={`w-full rounded-lg border-2 transition-all duration-150 flex items-center justify-center aspect-square text-xs font-mono font-bold
                    ${on
                      ...
                      : "bg-slate-800 border-slate-600 text-slate-600 group-hover:border-slate-400"
                    }`}
                >
                  {on ? "1" : "0"}
                </div>
                {/* ON / OFF label */}
                <span className={`text-[9px] uppercase font-semibold tracking-wide ${on ? "text-emerald-500" : "text-slate-600"}`}>
                  {on ? "on" : "off"}
                </span>
              </button>
            ))}
        ...
        {/* Result callout */}
        <div className="rounded-lg bg-slate-800 border border-slate-700 px-4 py-3 flex items-center justify-between gap-4">
          <div>
          ...
          <div className="text-right">
            ...
            <p className="font-mono text-sm text-slate-300 tracking-widest">
              {bits.map(b => b ? "1" : "0").join("")}
            </p>
            <p className="text-[10px] text-slate-600 mt-1">{onCount} of 8 switches on</p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
          <div className="flex justify-between text-xs text-slate-500">
            <span>0</span>
            <span className="text-slate-400">drag to change the number</span>
            <span>255</span>
          </div>
          <input
            type="range"
